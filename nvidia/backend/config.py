...
class Settings(BaseSettings):
    ...
    # ---------------------------------------------------------------------
    # Inference — NVIDIA backend variant (PROTECTED divergence file, see
    # scripts/sync-backends.sh). Nemotron 3 Nano 30B on SELF-HOSTED vLLM for
    # text/reasoning; Nemotron Nano VL via the NVIDIA NIM API for vision.
    # Blank fields take llm_client's Nemotron defaults.
    # ---------------------------------------------------------------------
    LLM_PROVIDER: str = Field(default="nemotron", description="Inference backend (nemotron)")
    DASHSCOPE_API_KEY: str = Field(default="", description="Unused on this backend (kept for .env compat)", repr=False)
    NVIDIA_API_KEY: str = Field(default="", description="NVIDIA NIM API key (vision side-path)", repr=False)
    LLM_BASE_URL: str = Field(default="", description="OpenAI-compatible base URL (blank = local vLLM :8001)")
    LLM_API_KEY: str = Field(default="", description="Bearer token for the vLLM endpoint (blank = EMPTY)", repr=False)
    LLM_MODEL: str = Field(default="", description="Text model id (blank = Nemotron-3-Nano-30B-A3B-BF16)")
    LLM_VISION_BASE_URL: str = Field(default="", description="Vision base URL (blank = NVIDIA NIM API)")
    LLM_VISION_MODEL: str = Field(default="", description="VLM for diagram analysis (blank = nemotron-nano-vl-8b)")
    ...
    # not comparable, so one model owns this backend's Supabase pgvector index.
    # Blank base URL falls back to the local vLLM embed server (:8003).
    EMBED_BASE_URL: str = Field(default="", description="Embeddings base URL (blank = local vLLM :8003)")
    EMBED_API_KEY: str = Field(default="", description="Embeddings bearer token (blank = EMPTY)", repr=False)
    EMBED_MODEL: str = Field(default="nvidia/llama-nemotron-embed-1b-v2", description="Skill-retrieval embedder (Matryoshka, 1024-d)")
    EMBED_DIMENSIONS: int = Field(default=1024, description="Must match the pgvector column width")
    # llama-nemotron-embed-1b-v2 is Matryoshka-trained; llm_client truncates and
    # re-normalizes client-side if the server rejects the `dimensions` param.
    EMBED_SEND_DIMENSIONS: bool = Field(default=True, description="Send `dimensions` in embedding requests")

    # Supabase skills store (this backend's learned-skills memory)
    SUPABASE_URL: str = Field(default="", description="Supabase project URL (skills store)")
    SUPABASE_SERVICE_KEY: str = Field(default="", description="Supabase service-role key", repr=False)
    ...
    def validate_llm_provider(cls, v: str) -> str:
        if v.strip().lower() != "nemotron":
            raise ValueError(f"Invalid LLM_PROVIDER: {v}. Only 'nemotron' is supported on the NVIDIA backend.")
...
try:
    ...
    print("\nKey environment variables (NVIDIA backend — Nemotron on self-hosted vLLM):")
    print("  - LLM_BASE_URL   (blank = http://localhost:8001/v1, the port-forwarded vLLM)")
    print("  - LLM_MODEL      (blank = nvidia/NVIDIA-Nemotron-3-Nano-30B-A3B-BF16)")
    print("  - NVIDIA_API_KEY (NIM API key for the vision side-path)")
    print("  - EMBED_MODEL (default: nvidia/llama-nemotron-embed-1b-v2), EMBED_DIMENSIONS (1024)")
    print("  - SUPABASE_URL / SUPABASE_SERVICE_KEY (skills store)")
...
def validate_configuration():
    ...
    print(f"   NIM key set (vision): {'yes' if settings.NVIDIA_API_KEY else 'NO — set NVIDIA_API_KEY'}")
    print(f"   Text Model: {_resolve('LLM_MODEL')}")
    print(f"   Vision Model: {_resolve('LLM_VISION_MODEL')} (via NIM)")
