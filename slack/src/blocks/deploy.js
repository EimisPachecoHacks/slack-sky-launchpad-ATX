...
export function successMessage(session, data) {
  ...
  if (data.gitlab_mr_url) buttons.push({ text: '📎 Open GitLab MR', action_id: 'dep_open_mr', url: data.gitlab_mr_url });
  if (data.endpoint) buttons.push({ text: '🌐 Open Endpoint', action_id: 'dep_open_endpoint', url: data.endpoint });

  const cardOpts = {
    emoji: '✅',
    title: 'Deployment succeeded',
    subtitle: `${p.emoji} ${p.label} · ${session.deploy.region}`,
    body: `${nComps} components · est. ${cost}/mo`,
    buttons,
  };
  ...
  return {
    ...
    text: `✅ Deployment succeeded on ${p.label} (${session.deploy.region})${data.endpoint ? ` — ${data.endpoint}` : ''}`,
  };
}
...
export function failureMessage(session, errMessage, logs) {
  ...
  const cardOpts = {
    ...
    buttons: [{ text: '🔁 Try Again', action_id: 'dep_retry', value: session.sid, style: 'primary' }],
  };
  const extra = [];
