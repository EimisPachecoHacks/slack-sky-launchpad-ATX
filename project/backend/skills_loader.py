"""
GitLab Duo Agent Platform — Skills Loader

Loads the workspace agent skills (SKILL.md files) that provide domain-specific
knowledge context for the IaC Generator agent. These are the same skills
referenced in the Duo Flow via `context:inputs.workspace_agent_skills`.
...
def get_skills_context(provider: str = "gcp") -> str:
    ...
    This mirrors the `workspace_agent_skills` input that the Duo Flow's
    iac_generator agent component receives.
    """
    ...
    sections = [
        "=== WORKSPACE AGENT SKILLS (GitLab Duo Agent Platform) ===",
        "",
